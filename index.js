// Carga del CSV y construcción del sunburst
d3.csv("customer_shopping_data.csv").then(data => {
  const hierarchyData = buildHierarchy(data);
  const chartNode = chart(hierarchyData);
  document.getElementById("chart").appendChild(chartNode);
});

// Agrupa datos por gender → age range → category
...
// Convierte edad numérica a rangos
...
// Dibuja el sunburst chart (basado en tu script)
function chart(data) {
  const width = 928;
  const height = width;
